...
```bash
cd "$INSTALL_DIR"
_O=$(git remote get-url origin 2>/dev/null || echo "")
_U=$(git remote get-url upstream 2>/dev/null || echo "")
if [ -n "$_U" ] && [ "$_U" != "$_O" ]; then echo "FORK_INSTALL=yes"; else echo "FORK_INSTALL=no"; fi
```

**`FORK_INSTALL=no`** (the common case) — nothing to do, go to Step 4.

**`FORK_INSTALL=yes`** — you MUST read `$INSTALL_DIR/gstack-upgrade/FORK-SYNC.md`
and follow it before Step 4. It rebases the carried patch onto the latest
upstream, and defines two outcomes (`conflict`, `push-failed`) that STOP the
upgrade: running Step 4's `reset --hard origin/main` after either would discard
the user's work. Do not improvise this step from memory.
